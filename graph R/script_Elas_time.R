...
p <- p + scale_y_continuous(breaks=seq(0,5,0.5))
#p <- p + scale_y_continuous(breaks=seq(0,250,25))
...
#p <- p + scale_fill_brewer(name="", labels=c("D&C"))
p <- p + geom_line(data=tab_ideal, aes(x=as.factor(Cores), y=value, group=1, color=variable), size=1)
p <- p + geom_point(data=tab_ideal, aes(x=as.factor(Cores), y=value, group=1, color=variable), size=2)
p <- p + theme_bw() + theme(legend.key = element_blank())

pdf(paste(f,".pdf",sep=""), width=4, height=2)
